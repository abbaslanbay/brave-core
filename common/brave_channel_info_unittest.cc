...
TEST(BraveChannelInfoTest, ParentDirectoryOfUserDataDirectoryTest) {
  ...
  EXPECT_EQ("MinegoInc", path.DirName().BaseName().AsUTF8Unsafe());
}
...
TEST(BraveChannelInfoTest, DefaultUserDataDirectoryAndChannelTest) {
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Minego-Browser", path.BaseName().AsUTF8Unsafe());
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Minego-Browser-Beta", path.BaseName().AsUTF8Unsafe());
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Minego-Browser-Dev", path.BaseName().AsUTF8Unsafe());
  ...
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Minego-Browser-Nightly", path.BaseName().AsUTF8Unsafe());
  EXPECT_EQ(version_info::Channel::CANARY, chrome::GetChannel());
  EXPECT_EQ(BRAVE_LINUX_CHANNEL_NIGHTLY,
            chrome::GetChannelName(chrome::WithExtendedStable(false)));
#else   // OFFICIAL_BUILD
  EXPECT_TRUE(chrome::GetDefaultUserDataDirectory(&path));
  EXPECT_EQ("Minego-Browser-Development", path.BaseName().AsUTF8Unsafe());
